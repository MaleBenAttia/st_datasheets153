"""
_run_poc.py — picks 2 PDFs per family, runs extraction + debug report.
Usage: python _run_poc.py
"""
...
# Pick 1 PDF from first 2 families only (fast test)
import random
rng = random.Random(42)

pdfs_to_run = []
for fam in families[:2]:  # C0, C5 only
    fam_dir = DATASHEETS / fam
    pdfs = sorted(fam_dir.glob("*.pdf"))
    if len(pdfs) >= 2:
        selected = rng.sample(pdfs, 2)
    elif len(pdfs) == 1:
        selected = pdfs
    else:
        continue
...
print(f"POC: {len(pdfs_to_run)} PDFs from 2 families")
...
print(f"\nPOC extraction: {ok} OK / {failed} FAILED in {elapsed:.0f}s")
